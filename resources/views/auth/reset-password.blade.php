...
@section('title', 'Reset Password — Jelajah Madura')
...
<div class="min-h-[70vh] flex items-center justify-center p-4">
    <div class="bg-white p-8 rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-gray-100 w-full max-w-md">
        <h1 class="text-2xl font-bold text-[#0f172a] mb-2">Reset Password</h1>
        ...
        <form method="POST" action="{{ route('password.update') }}" class="space-y-4">
            ...
            <div>
                ...
                <input id="email" type="email" name="email" value="{{ old('email', $email) }}" required readonly
                       class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-100 text-gray-500 text-sm focus:outline-none">
            </div>

            <div>
                <label for="password" class="block text-sm font-bold text-[#0f172a] mb-1.5">Password Baru</label>
            ...
            <div>
                <label for="password_confirmation" class="block text-sm font-bold text-[#0f172a] mb-1.5">Konfirmasi Password</label>
                <input id="password_confirmation" type="password" name="password_confirmation" required
                       class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#ff7b00]/20 focus:border-[#ff7b00] transition-all">
            </div>

            <button type="submit" class="w-full py-3 px-4 bg-[#ff7b00] hover:bg-[#e66f00] text-white font-bold rounded-xl transition-all mt-4">
